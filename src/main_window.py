...
from PyQt6.QtWidgets import (
    ...
    QMenu, QSystemTrayIcon, QMessageBox, QApplication
)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.records_list.itemClicked.connect(self.handle_item_click)
    ...
    def handle_item_click(self, item):
        ...
        try:
            ...
            # 更新图片预览的选中状态（不重新加载整个列表）
            if record['type'] == 'image':
                widget = self.records_list.itemWidget(item)
                if widget:
                    # 更新widget样式（与_create_image_preview保持一致，移除边框）
                    is_selected = record_id in self.selected_items
                    if is_selected:
                        widget.setStyleSheet(f"background-color: {highlight_color.name()}; border: none; border-radius: 0px;")
                    else:
                        widget.setStyleSheet(f"background-color: {bg_color.name()}; border: none; border-radius: 0px;")
        except Exception as e:
            print(f"处理项目点击时出错: {e}")
    ...
    def show_text_context_menu(self, position, content):
        ...
        copy_action.triggered.connect(lambda: self.copy_text_to_clipboard(content))
